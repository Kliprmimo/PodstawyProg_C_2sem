...
void bernoulli_gen(int v[], int n, double probability) {
	for( int i = 0; i < n; i++){
		v[i] = rand() < probability*(RAND_MAX+1);
	}
}
...
void cdf(double v[], int trials) {
	pmf(v, trials);
	for(int i = 2; i< TWO_DICE_SUM + 1; i++){
		v[i+1]+=v[i];
	}
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
	for(int i= x_start; i < 10; i++){
		printf(" %d|",i);
		for(int j = 0; j < v[i]/y_scale; j++){
			printf("%c", mark);
		}
		printf(" %.3f\n", v[i]);
	}
...
void monty_hall(int n, int *p_switch_wins) {
	for(int i = 0; i <n; i++){
		// kod ponizej w sprawdza problem zadany w poleceniu poprzez wylosowanie dwoch liczb modulo 3,
		// jesli wylosujemy te same dzwi za oboma razami to nie wygrywamy, poniewarz zawsze zmieniamy drzwi
	if(rand()%3 != rand()%3){
		*p_switch_wins += 1;
	}
	}
